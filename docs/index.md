...
`encoding/ascii85`, `unicode/utf8`, `hash/adler32`, `hash/crc32`, `hash/crc64`,
`strconv`, `encoding/json.Valid`, ASCII case folding, and the SIMD-amenable
primitives under and around them — **21 repositories** in all, plus a
proof-of-concept **7th (wasm-SIMD v128) target** for `matchlen` in
[`matchlen-wasm`](https://github.com/go-simd/matchlen-wasm) (not a kernel
...
**ppc64 (big-endian)**, on real POWER9. **Most repos are now natively measured
on real IBM z15** (VXE2, 2026-07-03, `-count=6`) — the vector-facility kernels
post clear wins (bitpack ~34×, streamvbyte decode ~20×, hex encode ~18×,
matchlen ~9.2×, crc64 ~9.1×). Two repos — `jsonvalidate` and `ascii85` — still
carry the older qemu-validated-for-correctness framing, native z15 throughput
pending for those two. Headline numbers come from native measurement and are
never extrapolated. Two results stand out: **base32 gets real SIMD on ppc64le
...
| [`crc32`](repos/crc32.md) | `hash/crc32` | **arm64-only, deliberately**: stdlib IEEE path is already hardware-assisted elsewhere; `PMULL`/`PMULL2` fold-by-eight beats arm64's serial `CRC32X` |
...
Five repos — [`ascii`](repos/ascii.md), [`levenshtein`](repos/levenshtein.md),
[`bitset`](repos/bitset.md), [`int8dot`](repos/int8dot.md) and
[`jsonvalidate`](repos/jsonvalidate.md) — rounded the suite out to **20**, and
[`crc32`](repos/crc32.md) makes **21**. [`jsonvalidate`](repos/jsonvalidate.md)
is the only SIMD JSON validator covering all six arches (byte-identical to
`encoding/json.Valid`); [`levenshtein`](repos/levenshtein.md) is honestly
*pure-Go bit-parallel*, **not vector-SIMD** — its word-serial Myers column has
no lane parallelism (documented) — yet still **62–142× vs `agnivade`**;
[`bitset`](repos/bitset.md) and [`int8dot`](repos/int8dot.md) are vector
primitives (int8dot's arm64 NEON kernel needs **Go 1.27**, scalar below);
[`ascii`](repos/ascii.md) runs real SIMD case-folding on arm64 too, via a
multiply-free sign-bit predicate (**~4.9× stdlib**); and
[`crc32`](repos/crc32.md) is the deliberate exception to the six-arch pattern
— arm64-only, since stdlib already hardware-accelerates IEEE crc32 elsewhere.
As elsewhere in the org, **ppc64le, riscv64 and loong64 are now natively
measured on real silicon** (GCC Compile Farm — POWER9, SpacemiT X60 RVV 1.0,
Loongson 3A5000); **most repos are now natively measured on real IBM z15**
(VXE2, 2026-07-03) — `jsonvalidate` and `ascii85` are the two still pending a
follow-up z15 run — see the per-repo pages for the vector-facility speedups.
