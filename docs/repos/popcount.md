...
| others | portable `math/bits.OnesCount64` word loop |
...
loong64 under QEMU). All three amd64 dispatch branches (hardware POPCNT, AVX2
fallback, scalar) are exercised on the native amd64 runner by toggling the feature
flags. The `.s` kernels are validated by the differential force tests against the
scalar reference plus the fuzz target. BSD-3-Clause.
